...
const services = [
  { to: "/personaltraining", label: "Personal Training", num: "01", desc: "1:1 Betreuung mit voller Aufmerksamkeit." },
  { to: "/gruppentraining", label: "Gruppentraining", num: "02", desc: "Kleine Gruppen, gemeinsamer Antrieb." },
  { to: "/athletiktraining", label: "Athletiktraining", num: "03", desc: "Periodisierte Athletik für Mannschaften." },
  { to: "/physiotherapie", label: "Physiotherapie", num: "04", desc: "Ursache finden, schmerzfrei werden." },
  { to: "/trainingstherapie", label: "Trainingstherapie", num: "05", desc: "Aktives Aufbautraining nach Verletzungen." },
  { to: "/outdoor-training", label: "Outdoor Training", num: "06", desc: "Frische Luft, ganzheitliche Bewegung." },
] as const;
        ...
        <div className="grid md:grid-cols-2 gap-px bg-foreground/10 border border-foreground/10 not-prose">
          {services.map((s) => (
            <Link
              key={s.to + s.label}
              to={s.to}
              className="bg-background p-8 hover:bg-muted group"
            >
              <div className="font-mono text-[11px] text-primary mb-3">[ {s.num} ]</div>
              <div className="font-display text-2xl mb-2 group-hover:text-primary">
                {s.label}
              </div>
          ...
          >
            <div>
              <div className="font-mono text-[11px] mb-3 opacity-80">[ 07 ]</div>
