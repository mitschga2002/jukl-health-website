...
**Source:** downloaded from the Lovable CDN that serves the deployed site
(`https://juklhealth.com/__l5e/assets-v1/<uuid>/<name>`), which was the only
copy of these files. They are not referenced by application code — the site
loads the derived variants from `public/img/` — but they are the input for
regenerating those variants.

**Known limitation:** these are 1385–1824 px wide. They were already downscaled
by the upload pipeline before reaching the CDN (camera EXIF stripped; where
